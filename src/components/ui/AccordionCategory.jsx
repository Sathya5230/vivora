...
export default function AccordionCategory({
  ...
  return (
    <div className="rounded-[1.75rem] border border-plum-100 bg-white">
      <button
        type="button"
      ...
      >
        <span className="flex items-center gap-3">
          ...
          <span className="font-mono text-xs text-plum-700/30">
      ...
      {isOpen && (
        <div className="flex flex-wrap gap-2 px-6 pb-6">
          {visibleEvents.map((eventName) => (
            <span
              key={eventName}
              className="rounded-full bg-plum-50 px-4 py-2 text-sm text-plum-700"
            >
              {eventName}
            </span>
          ))}
